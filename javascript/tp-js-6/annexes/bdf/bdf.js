...
var sans_telephone=societes.filter((s)=>s[1]==="xx-xx-xx-xx-xx").length;
// A COMPLETER
...
societes.forEach(function(s){
  let departement=Math.floor(s[2]/1000);
  if(departements.includes(departement)){
    departement in distribution ?
    (distribution[departement]+=1): (distribution[departement]=1);
  }
});
// A COMPLETER
...
var prenoms=societes.map((s)=>s[4].split(" ")[0]).supprimerDoublons().sort();
...
societes.forEach(function(s){
  ...
  tr.insertCell().textContent=s[2];
  tr.insertCell().textContent=s[4];

  prenoms_feminins.map(function(f){
    if(s[4].match(new RegExp(f))){
      tr.lastElementChild.classList.add("presidente");
    }
  });
  
});
function Societe(nom, cp, president){
  this.nom=nom;
  this.cp=cp;
...
Societe.prototype.toHTML=function(datalist){
  ...
  option.value=this.nom+ "("+this.cp+ ")"+this.president;
